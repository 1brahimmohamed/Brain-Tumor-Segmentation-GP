...
export default class Matrix {
    ...
    minor(i: number, j: number) {
        const newRows = HelpersUtil.withoutElementAtIndex(this.rows, i)
            .map((row) => HelpersUtil.withoutElementAtIndex(row, j))
    ...
    adjugate() {
        return this
            .map((_: number, i: number, j: number) => this.cofactor(i, j))
            .transpose()
    }
