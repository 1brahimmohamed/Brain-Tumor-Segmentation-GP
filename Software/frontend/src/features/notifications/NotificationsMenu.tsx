...
const NotificationsMenu = () => {
    ...
    return (
        <div className={'h-auto w-80 flex-col space-y-1'}>
            {notifications.length === 0 ? emptyNotifications() : notifications.map((notification) => {
                return (
                        <NotificationItem id={notification.id} content={notification.content} type={notification.type} />
                );
            })}
        </div>
    );
