...
const filterStudiesReducers = {
    setDateFilter: (state: IStoreStudiesSlice, action: PayloadAction<{startDate: string | null, endDate: string | null }>) => {
    ...
    setFilterPeriod: (state: IStoreStudiesSlice, action: PayloadAction<string>) => {
        ...
        switch (action.payload) {
            case '1d':
                state.startDateFilter = new Date(today.getFullYear(), today.getMonth(), today.getDate() - 1).toISOString()
                state.endDateFilter = new Date().toISOString();
                break;
            case '3d':
                state.startDateFilter = new Date(today.getFullYear(), today.getMonth(), today.getDate() - 3).toISOString();
                state.endDateFilter = new Date().toISOString();
                break;
            case '1w':
                state.startDateFilter = new Date(today.getFullYear(), today.getMonth(), today.getDate() - 7).toISOString();
                state.endDateFilter = new Date().toISOString();
                break;
            case '1m':
                state.startDateFilter = new Date(today.getFullYear(), today.getMonth() - 1, today.getDate()).toISOString();
                state.endDateFilter = new Date().toISOString();
                break;
            case '1y':
                state.startDateFilter = new Date(today.getFullYear() - 1, today.getMonth(), today.getDate()).toISOString();
                state.endDateFilter = new Date().toISOString();
                break;
            default:
