...
from model.v2.nvidia_nnunet import *
...
class InteractiveSegmentation(ScriptedLoadableModule):
    ...
    def __init__(self, parent):
        ...
        self.parent.title = "Interactive Segmentation"  # TODO: make this more human readable by adding spaces
        self.parent.categories = ["MMMAI"]  # TODO: set categories (folders where the module shows up in the module selector)
...
class InteractiveSegmentationWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def setup(self):
        ...
        # Buttons
        self.ui.comboBox.addItems(['None', 'nnUNet', 'Nvidia-nnUNet', '3D-UNet'])
        ...
        with slicer.util.tryWithErrorDisplay("Failed to compute results.", waitCursor=True):
            ...
            if modelname != 'None':
                ...
                # Get a list of all scalar volume nodes in the scene
                    ...
                    input_folder =  os.path.join(current_file_dir_path, "input")
                    ...
                    try:
                        predict_nnunet(f"{input_folder}",self.output_directory)
                    ...
                    self.output_directory = os.path.dirname(volume_nodes[0].GetStorageNode().GetFileName()) # path of the folder of uploaded files on slicer
                    current_file_dir_path = os.path.dirname(os.path.abspath(__file__)) # ../MMMAI/InteractiveSegmentation
                    root_path = os.path.dirname(current_file_dir_path)  #../MMMAI
                    BraTS2021_train_path =  os.path.join(current_file_dir_path, "data","BraTS2021_train") #../MMMAI/InteractiveSegmentation/data/BraTS2021_train
                ...
                if modelname == 'Nvidia-nnUNet':
                    ...
                    try:
                        ...
                        print("bykml")
                        nvidia_preprocess(1,root_path)
                        nvidia_predict(root_path)
                        # prepare_prediction(1,self.output_directory,root_path)
                ...
                if modelname == '3D-UNet':
                    print("3D-UNet")
                    try:
                        # prepare_dataset(input_folder_path, True)
                        # nvidia_preprocess(0,root_path)
                        # predict_3dunet(root_path)
                        # prepare_prediction(0,self.output_directory,root_path)
                        print("Finished 3D-UNet!")
                    except Exception as e:
                        print(f"An error occurred: {e}")    
...
#
# InteractiveSegmentationTest
#

class InteractiveSegmentationTest(ScriptedLoadableModuleTest):
    """
