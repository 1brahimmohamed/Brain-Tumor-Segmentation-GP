...
def run_nnUNet_predict(input_dir, output_dir, task, checkpoint, model_type):
    ...
    slicer_directory = os.path.dirname(os.path.dirname(sys.executable))
    # pip install nnunet must be executed in python console
    nnUNet_predict_path = os.path.join(slicer_directory, 'lib/Python/bin/nnUNet_predict')
    command = [
        nnUNet_predict_path,
        "-i", f'{input_dir}',
        "-o", f'{output_dir}',
