...
def nvidia_preprocess(n,root_path):
    # n --> to specify is it for nnunet(1) or 3d unet(0)
    # needs generalization
    print("nvidia_preprocess")
    print(root_path)
    preprocess_path = os.path.join(root_path, 'DeepLearningExamples/PyTorch/Segmentation/nnUNet/preprocess.py')
    data_folder_path = os.path.join(root_path, 'InteractiveSegmentation/data')
...
def nvidia_predict(root_path):
    ...
    python_path = sys.executable
    nvidia_main_path = os.path.join(root_path, 'DeepLearningExamples/PyTorch/Segmentation/nnUNet/main.py')
    preprocessed_data_path = os.path.join(root_path, 'InteractiveSegmentation/data/11_3d')
    ckpt_path = os.path.join(root_path, 'InteractiveSegmentation/nvidia_util/epoch=7-dice=84.40.ckpt')
    results_path_npy = os.path.join(root_path, 'InteractiveSegmentation/data')
...
def predict_3dunet(root_path):
    print("predict_3dunet")

    python_path = sys.executable

    nvidia_main_path = os.path.join(root_path, 'DeepLearningExamples/PyTorch/Segmentation/nnUNet/main.py')
    preprocessed_data_path = os.path.join(root_path, 'InteractiveSegmentation/data/11_3d')
    ckpt_path = os.path.join(root_path, 'InteractiveSegmentation/nvidia_util/last.ckpt')
    results_path_npy = os.path.join(root_path, 'InteractiveSegmentation/data')
...
def save_nifti_prediction(prediction_tuple,output_directory,root_path):
    ...
    original_image_path = os.path.join(root_path,f"InteractiveSegmentation/data/BraTS2021_train/images/{fname}.nii.gz")
...
def prepare_prediction(model_switch,output_directory,root_path):

    print('prepare_prediction')
    # model_switch 0 for 3dunet 1 for nvidia nnunet
    if (model_switch):
         input_path =os.path.join(root_path,"InteractiveSegmentation/data/predictions_epoch=7-dice=84_40_task=11_fold=0_tta")
        # input_path = "/home/mahamedhat/nvidia-Segmentation/Seg/data/predictions_epoch=7-dice=84_40_task=11_fold=0_tta"
    else:

        input_path =os.path.join(root_path,"InteractiveSegmentation/data/predictions_last_task=11_fold=0_tta")
        # input_path = "/home/mahamedhat/nvidia-Segmentation/Seg/data/predictions_last_task=11_fold=0_tta"   
